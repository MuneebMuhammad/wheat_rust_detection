...
class SimSiamDataset(Dataset):
    ...
    Args:
        path (str): Path to patches
        valid_exts (List[str]): List of valid patches extensions
        size (int): Desired image size
        normalize (bool): Whether to normalize with ImageNet statistics
    """
    def __init__(self, path: str = 'images/', 
                 valid_exts: List[str] = ['jpeg', 'jpg', 'JPG'],
                 size: int = 224, normalize: bool = True):
        self.files = []
        if path == 'train':
            self.files = glob.glob('/scratch/muneebm/all_patches/*/*')
            print("train files:",len(self.files))
        else:
            self.files = glob.glob('/scratch/muneebm/val_data_300_patch/*/*')
            print("val files: ", len(self.files))
