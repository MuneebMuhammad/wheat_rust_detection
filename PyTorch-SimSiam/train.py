...
WEIGHT_LOC = '/scratch/muneebm/weights/SimSiam_dict'
graph_file = '/home/muneebm/work/PyTorch-SimSiam/outputs/metrics.csv'
os.makedirs(WEIGHT_LOC, exist_ok=True)

with open(graph_file, 'w') as f:
    writer = csv.writer(f)
    writer.writerow(['loss', 'std1', 'std2'])
...
def train(train_dataloader: DataLoader, valid_dataloader: DataLoader, 
    ...
    for epoch in range(n_epochs):
        ...
        torch.save(model.state_dict(), WEIGHT_LOC + '/{}.pth.tar'.format(epoch))

        with open(graph_file, 'a') as f:
            writer = csv.writer(f)
            writer.writerow([total_loss, total_std1, total_std2])
