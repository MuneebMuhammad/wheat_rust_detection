...
graph_file = "/home/muneebm/work/WheatDisease/outputs/10_epoch_curriculum/simsiam_pretrained_resnet18_unet_easy_medium_RGBThresh_curriculum_300_patch.csv"  # for simsiam pretrained
# graph_file = "/home/muneebm/work/WheatDisease/outputs/10_epoch_curriculum/just_resnet18_unet_easy_medium_RGBThresh_curriculum_300_patch.csv" # for just resnet
PATCH_SIZE = 300
unet_input_channels = 4
resnet_weights_directory_list = ["/scratch/muneebm/weights/simsiam_pretrained_resnet18_curriculum_patches_224_patch/12.pth.tar"]
unet_weights_directory_list = [i for i in glob.glob('/scratch/muneebm/weights/10_epoch_curriculum/easy_medium_unet/*') if int(i.split('/')[-1].split('.')[0]) %2 == 0]
testing_directory = "/scratch/muneebm/10_epoch_curriculum/simsiam_resnet18_unet_easy_medium"  # for simsiam pretrained
# testing_directory = "/scratch/muneebm/10_epoch_curriculum/just_resnet18_unet_easy_medium"  # for resnet
...
resnet_model.fc = nn.Linear(512, 2) # for resnet18
# resnet_model.fc = nn.Linear(2048, 2) # for resnet50

unet_model = torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet',
                            in_channels=unet_input_channels, out_channels=1, init_features=32, pretrained=False)
...
# create list of patch pixel level indices
...
# performs 3 step model, and return f1 scores
def f1_score_unet(patches, input_img, thresh_mask, label):
    ...
    for p_num, patch in enumerate(patches):
        tp, tn, fn, fp = 0, 0, 0, 0
        # step 2 of technique: Resnet18 classifier
        classifier_pred = (resnet_model(
            transform_val_resnet(input_img[patch[1]:patch[1] + PATCH_SIZE, patch[0]:patch[0] + PATCH_SIZE]).unsqueeze(
                0).to(device)).argmax(axis=1) == 1)
        if ((thresh_mask[patch[1]:patch[1] + PATCH_SIZE,
             patch[0]: patch[0] + PATCH_SIZE].max() == 255) and classifier_pred):
            thresh_patch = transform_val_unet(
                thresh_mask[patch[1]:patch[1] + PATCH_SIZE, patch[0]: patch[0] + PATCH_SIZE])
            patch_input_image = transform_val_unet(
                input_img[patch[1]:patch[1] + PATCH_SIZE, patch[0]:patch[0] + PATCH_SIZE])
            combined_input = torch.concat((patch_input_image, thresh_patch), axis=0)
            combined_input = combined_input.unsqueeze(0)
            # Step 3 of technique: Unet with RGBT channel input
            pred = torch.round(unet_model(combined_input.to(device)))
...
for resnet_model_name in resnet_weights_directory_list:
    for unet_model_name in unet_weights_directory_list:
        ...
        # resnet_model.load_state_dict(torch.load(resnet_model_name), strict=False)
        ...
        for l, r in zip(masks, rusts):
            ...
            # Step 1 of technique: creating thresholding mask with different color spaces
